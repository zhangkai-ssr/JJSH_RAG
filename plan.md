...
| M7 BOM、网表和 PDF | 进行中 | 本地 feature 已完成解析、定位、实际索引、46 条评估和首轮独立 review 修正；待复审、PR 合并及 `main` 复验后才能写“已满足” |
...
| 2026-09-01 | worktree / M7 | `feature/m7-structured-sources` / `.WORKTREE/m7-structured-sources/` | 受控准入4份BOM XLSX、4份Protel网表和9份原理图/PCB PDF；增加工作表单元格、网表行号和PDF页码定位；保持commit、SHA-256、状态和证据等级；补充6条结构化黄金问题；首轮独立 review 后修正结构化来源 fail-open、文本 chunk ID 兼容、格式边界实检、PDF 恢复审计和优先级越位。 | 源提交`f89e2f847998ee3992c432ccfd2b6df8aa4eca63`；实际索引351文档/4548 chunks/0 errors；1份源PDF触发2条可审计恢复warning；78项测试通过；46条评估版本污染0，引用、定位、证据、拒答和边界100%，Top 5来源97.62%；未修改源仓库或访问设备。 | 本地提交，未推送 / 未建 PR | 进行中 |

## 后续准入顺序

1. M7 本地实现及首轮独立 review 修正已形成；下一门禁是复审、用户分别授权推送/建 PR/合并，以及合并后的 `main` 重建和复验。
