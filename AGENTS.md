...
4. PR 前检查 freshness、完整 diff 和验证证据。环境支持独立 reviewer 时执行独立 review；不支持时
   如实记录限制，不得伪称已完成独立评审。
5. 推送、创建 PR、合并或删除 worktree/分支前分别取得用户授权，并再次核对精确远程与目标分支。
6. PR 创建后保留 worktree。只有 PR 合并、`main` 复验和 `plan.md` 状态更新完成后，才从主工作区
   删除该 worktree、prune 并删除 feature 分支。
7. 禁止用本地合并绕过用户选择和 PR 状态，也禁止强推覆盖远程历史。
