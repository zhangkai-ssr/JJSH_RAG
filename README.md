...
当前只索引 `C:\work1\JSZN\ESP32_S3` 中 Git 跟踪、可归属到提交的
`1.6_R6` 正式文本和受控 BOM、网表、PDF，并在回答中保留版本、源提交、文件哈希、
证据等级及原格式定位符。
...
- 受控解析 `BOM_*.xlsx`、`Netlist_*.tel`、原理图 PDF 和 PCB PDF；不自动纳入贴片坐标或其他二进制交付物。
...
| `src/jssh_rag/structured.py` | XLSX BOM 与 PDF 的格式原生解析和定位 |
| `tests/` | 自动化测试 |
| `evals/` | 黄金问题集和评估结果 |
...
需要 Python 3.11 或更高版本；PDF 文本提取使用 `pypdf>=6,<7`，安装命令会一并处理依赖。
...
索引器通过 Git 读取文件清单和源提交，只纳入已跟踪文件；BOM、网表和 PDF 还必须匹配
来源策略中的精确目录/文件名模式。发现已跟踪但未提交的
...
jssh-rag search --version 1.6_R6 --query "LIS2MDLTR U10 C919695"
jssh-rag search --version 1.6_R6 --query "LIS2_DRDY CN1.11 CN2.11"
...
文本引用使用 `start_line/end_line`；BOM 使用 `工作表!单元格范围`，PDF 使用 `page N`。
PDF 只索引文件中可提取的文字，不推断图形连线、铜皮或 PCB 连通性。BOM/网表/PDF 的
`source_reviewed` 只表示受控源文件内容可复核，不等于板厂生产授权、实板导通或真机验收。
含 BOM、网表或 PDF 证据的回答固定使用本地确定性摘要，不把结构化证据发送给私有 LLM；
普通文本来源仍可在显式配置后使用私有 LLM。格式边界和生成后检查继续防止把导出内容提升为
实物装配、实板导通、图形连通性验证或生产授权。
若 PDF 需要宽松恢复才能读取，索引报告会把解析器诊断列入 `warnings`；这类文件可以继续
提供可检索文字，但不能被表述为严格解析通过，宜回到源仓库重新导出或修复。
...
索引，避免复用不兼容的向量缓存。结构化来源使用确定性回答，不发送给 LLM 服务。
...
M7 BOM、网表和 PDF 完成合并与干净检出复验后，再评估其他硬件版本。设备控制代理不属于
本仓库当前范围；如未来立项，应与只读知识库分仓、
分权限和分验收链路实施。
